#!/usr/bin/env python3
"""v3 diagnostic: Use sidebar navigation for SPA routes."""
...
def click_sidebar(page, label: str):
    """Click a sidebar item by visible text or title."""
    selectors = [
        f'a:has-text("{label}")',
        f'text="{label}"',
        f'[title="{label}"]',
        ...
        if loc.count() > 0:
            try:
                ...
                for i in range(loc.count()):
                    item = loc.nth(i)
                    if item.is_visible():
                        print(f"Clicking sidebar '{label}' with selector {sel} (nth={i})")
                        item.click()
                        page.wait_for_timeout(3000)
                        return True
            except Exception as e:
                print(f"Click failed for {sel}: {e}")
    print(f"WARNING: Could not find sidebar item '{label}'")
    return False
...
def extract_applications(page):
    ...
    click_sidebar(page, "Applications")
    page.wait_for_timeout(4000)

    # Also try direct hash as backup
    page.goto(f"{BASE_URL}/app/#/agent/applications", wait_until="networkidle", timeout=30000)
    page.wait_for_timeout(3000)

    shot_path = REPORTS_DIR / "applications_v3.png"
...
def extract_conversations(page):
    print("\n=== CONVERSATIONS ===")
    # First go back to dashboard to reset
    click_sidebar(page, "Dashboard")
    page.wait_for_timeout(3000)

    # Click Conversations in sidebar
    click_sidebar(page, "Conversations")
    page.wait_for_timeout(4000)

    # Also force hash change via JS
    page.evaluate("window.location.hash = '#/agent/conversations'")
    page.wait_for_timeout(3000)

    shot_path = REPORTS_DIR / "conversations_v3.png"
    page.screenshot(path=str(shot_path), full_page=True)
    print(f"Screenshot: {shot_path}, URL: {page.url}")

    convs = []
    # Ember might render in a specific container
    selectors = [
        "table tbody tr",
...
def run():
    with sync_playwright() as pw:
        ...
        # Dashboard screenshot to understand layout
        page.goto(f"{BASE_URL}/app/#/agent/dashboard", wait_until="networkidle", timeout=30000)
        page.wait_for_timeout(4000)
        shot_path = REPORTS_DIR / "dashboard_v3.png"
        page.screenshot(path=str(shot_path), full_page=True)
        print(f"Dashboard screenshot: {shot_path}")
